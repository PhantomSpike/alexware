function make_simlnp_model(model,h_max_ms,dt_ms,normz,save_dir,kfold,n_cores)
%% Input params
...
cluster_dir = '/mnt/40086D4C086D41D0/Reverb_neuronal_data/Kernel_fits/kfolds/LNP_model/perfreq/ridge/10ms/Predicted_PSTHs'; %Directory with all the clusters
...
parfor c = 1:n_clusters
    ...
    temp = load(fullfile(cluster_dir,strjoin({animal_names{c},pen_names{c},num2str(cluster_ids(c))},'_')),'psth');
    
    if ismember(animal_names{c},["Ronnie","PLP"])
        kernels{c} =  gen_simlnp_kernel(coch1,temp.psth,h_max_ms,model,normz,kfold);
    elseif ismember(animal_names{c},["Cork","Kilkenny","Derry"])
        kernels{c} =  gen_simlnp_kernel(coch2,temp.psth,h_max_ms,model,normz,kfold);
    else
...
lnp_dir = fullfile(save_dir,'LNP_model_sim');
...
for c = 1:n_clusters
    kernel = kernels{c};
    kernel.params.animal_name = animal_names{c};
    kernel.params.pen_name = pen_names{c};
    kernel.params.cluster_id = cluster_ids(c);
    kernel.params.NPSP = NPSPs(c);
    kernel.params.quality = qualities{c};
    save(fullfile(save_dir_full,strjoin({animal_names{c},pen_names{c},num2str(cluster_ids(c))},'_')),'kernel');
end

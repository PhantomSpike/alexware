function make_neuronal_models(model,h_max_ms,dt_ms,normz,save_dir,kfold,n_cores)
%% Input params
...
parfor c = 1:n_clusters
    ...
    if ismember(temp.data.params.animal_name,["Ronnie","PLP"])
        kernels{c} =  gen_neuronal_kernel(coch1,temp.data,h_max_ms,model,normz,kfold);
    elseif ismember(temp.data.params.animal_name,["Cork","Kilkenny","Derry"])
        kernels{c} =  gen_neuronal_kernel(coch2,temp.data,h_max_ms,model,normz,kfold);
    else

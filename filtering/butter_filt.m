function [dataOut,zf] = butter_filt(dataIn,fs,low_cutoff,high_cutoff,stopband_wide_low,stopband_wide_high,zf)
...
[b,a] = butter(n,Wn);
if isempty(zf)
  [dataOut, zf] = filter(b,a,dataIn);
else
  [dataOut, zf] = filter(b,a,dataIn,zf);
end
